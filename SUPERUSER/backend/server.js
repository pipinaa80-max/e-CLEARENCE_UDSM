...
async function route(req, res) {
  ...
  try {
    ...
    if (url.pathname === '/api/global-branding' && req.method === 'PUT') { if (!requireRole(req, res, 'SUPERUSER')) return; const input = await body(req); const fonts = ['Georgia', 'Arial', 'Verdana', 'Trebuchet MS']; const branding = { ...globalBranding(data), universityName: String(input.universityName || data.branding.universityName).trim(), shortName: String(input.shortName || data.branding.shortName).trim(), primaryColor: /^#[0-9a-f]{6}$/i.test(input.primaryColor || '') ? input.primaryColor : data.branding.primaryColor, fontFamily: fonts.includes(input.fontFamily) ? input.fontFamily : data.branding.fontFamily }; data.branding = branding; for (const project of Object.values(data.projects || {})) project.branding = { ...project.branding, ...branding }; writeData(data); return send(res, 200, branding); }
    if (url.pathname === '/api/global-branding/logo' && req.method === 'POST') { if (!requireRole(req, res, 'SUPERUSER')) return; const input = await body(req); if (!/^data:image\/(png|jpeg|jpg|webp);base64,[A-Za-z0-9+/=]+$/.test(input.dataUrl || '') || Buffer.byteLength(input.dataUrl, 'utf8') > 3_000_000) return send(res, 400, { message: 'Upload a PNG, JPEG, or WebP image smaller than 2 MB' }); data.branding.logoData = input.dataUrl; data.branding.logoUrl = 'http://localhost:8090/api/public/logo'; for (const project of Object.values(data.projects || {})) { project.branding.logoData = input.dataUrl; project.branding.logoUrl = data.branding.logoUrl; } writeData(data); return send(res, 200, { logoUrl: data.branding.logoUrl }); }
