...
async function route(req, res) {
  ...
  try {
    if (req.method === 'POST' && url.pathname === '/api/login') {
      ...
      const identity = isSuperuser ? { role: 'SUPERUSER', email: data.superuser.email, permissions: ['ALL'] } : { role: 'PROJECT_ADMIN', email: admin.email, adminId: admin.id, permissions: admin.permissions };
      const token = crypto.randomBytes(32).toString('hex'); sessions.set(token, identity);
      return send(res, 200, { token, user: { email: identity.email, role: identity.role, permissions: identity.permissions } });
    }
    ...
    if (url.pathname === '/api/overview' && req.method === 'GET') { if (current.role === 'SUPERUSER') return send(res, 200, { role: current.role, branding: data.branding, dashboards: data.dashboards, subAdmins: data.subAdmins.map(safeAdmin) }); return send(res, 200, { role: current.role, branding: data.branding, dashboards: data.dashboards.filter(item => item.enabled), subAdmins: [] }); }
    if (url.pathname === '/api/branding' && req.method === 'PUT') { if (!requireRole(req, res, 'PROJECT_ADMIN')) return; const input = await body(req); const fonts = ['Georgia', 'Arial', 'Verdana', 'Trebuchet MS']; data.branding = { ...data.branding, universityName: String(input.universityName || data.branding.universityName).trim(), shortName: String(input.shortName || data.branding.shortName).trim(), logoUrl: String(input.logoUrl || '').trim(), primaryColor: /^#[0-9a-f]{6}$/i.test(input.primaryColor || '') ? input.primaryColor : data.branding.primaryColor, fontFamily: fonts.includes(input.fontFamily) ? input.fontFamily : data.branding.fontFamily }; writeData(data); return send(res, 200, data.branding); }
    ...
    if (url.pathname === '/api/sub-admins' && req.method === 'POST') { if (!requireRole(req, res, 'SUPERUSER')) return; const input = await body(req); if (!input.email || !input.name || !input.password) return send(res, 400, { message: 'Name, email, and password are required' }); if (data.subAdmins.some(s => s.email === input.email.trim().toLowerCase())) return send(res, 409, { message: 'Email already exists' }); const admin = { id: id(), name: input.name.trim(), email: input.email.trim().toLowerCase(), passwordHash: hashPassword(input.password), role: 'PROJECT_ADMIN', permissions: Array.isArray(input.permissions) ? input.permissions : [], active: true, createdAt: new Date().toISOString() }; data.subAdmins.push(admin); writeData(data); return send(res, 201, safeAdmin(admin)); }
