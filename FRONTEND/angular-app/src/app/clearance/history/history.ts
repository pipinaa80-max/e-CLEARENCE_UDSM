...
export class ClearanceHistoryComponent {
  ...
  get requests(): ClearanceRequest[] { const user = this.authService.getCurrentUser(); const requests = user ? this.clearanceService.getClearanceHistory(user.id) : []; return this.filter === 'All' ? requests : requests.filter((request: ClearanceRequest) => request.status === this.filter); }
